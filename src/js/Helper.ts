...
export class Helper {
                    ...
                    for (let i = 0; i < element.children.length; i++) {
                        ...
                        if (child) {
                            for (let j = 0; j < classArray.length; j++) {
                                if (child.classList.contains(classArray[j])) {
                                    classUsed = classArray[j];
                                    elm = child;
                                    break;
                                }
                            }
    ...
    public static attachEventListener(element: HTMLElement, event: Function, eventType: string) {
        element.addEventListener(eventType, () => {
            event();
        });
    }

    public static displayLog({ msg, error }: { msg: string, error?: any }) {
        if (error) {
            console.log(error);
        }
        console.warn(msg);
    }
    public static colorNameToHexCode(colorName : string) {
    ...
    public static validateUnit(unit : string) {
        let unitName = ["%", "px", "rem", "em"];
        if(unitName.indexOf(unit.toLowerCase()) < 0){
            return false;
        }else{
            return true;
        }
